...
/// \brief 析构函数
///
My_TreeView_Device::~My_TreeView_Device()
{
//    qDebug() << "~My_TreeView_Device";
}
...
/// \return
///
bool My_TreeView_Device::Add_LeftTree_Slaves(int master_count, const QString &slave_name,int input_count,int output_count){
//    static int s_slave_count = 1;
    ...
    if(masterFolder)
    {
        ...
        Add_LeftTree_SlaveItems(master_count,s_slave_count,MARK_SLAVE_ITEM_INPUT,input_count);//每次添加slave就添加一对输入输出
        Add_LeftTree_SlaveItems(master_count,s_slave_count,MARK_SLAVE_ITEM_OUTPUT,output_count);
...
/// \return
///
bool My_TreeView_Device::Add_LeftTree_SlaveItems(int master_count, int slave_count, int slaveItem,int item_count){//从站输入输出
...
//    return false;
    ...
    if(slaveFolder)
    {
        for(int indexItem=0;indexItem<item_count;indexItem++){//循环次数
            QStandardItem* item = new QStandardItem();
            switch(slaveItem){
            case MARK_SLAVE_ITEM_INPUT://输入
                if(item_count == 1){// 如果只是循环一次
                    item->setText(tr("Input"));
                }
                else{
                    item->setText(tr("Input")+QString::number(indexItem+1));
                }

                item->setIcon(QIcon(":/image1/Resource/Icons/Input.png"));

                item->setData(MARK_SLAVE_ITEM_INPUT+indexItem,ROLE_MARK);//标记为slave item
                item->setData(MARK_SLAVE_ITEM_INPUT+indexItem,ROLE_MARK_MASTER);//再次标定为master
                item->setData(MARK_SLAVE_ITEM_INPUT+indexItem,ROLE_MARK_SLAVE);//再次标定为master
                item->setData(MARK_SLAVE_ITEM_INPUT+indexItem,ROLE_MARK_SLAVE_ITEM);//再次标定为master
                slaveFolder->appendRow(item);
                break;
            case MARK_SLAVE_ITEM_OUTPUT://输出
                if(item_count == 1){// 如果只是循环一次
                   item->setText(tr("Output"));
                    ...
                    item->setText(tr("Output")+QString::number(indexItem+1));
                }

                item->setIcon(QIcon(":/image1/Resource/Icons/Output.png"));

                item->setData(MARK_SLAVE_ITEM_OUTPUT+indexItem,ROLE_MARK);//标记为slave item
                item->setData(MARK_SLAVE_ITEM_OUTPUT+indexItem,ROLE_MARK_MASTER);//再次标定为master
                item->setData(MARK_SLAVE_ITEM_OUTPUT+indexItem,ROLE_MARK_SLAVE);//再次标定为master
                item->setData(MARK_SLAVE_ITEM_OUTPUT+indexItem,ROLE_MARK_SLAVE_ITEM);//再次标定为master
                slaveFolder->appendRow(item);
                break;
            default:
                break;
...
bool My_TreeView_Device::Add_LeftTree_SlaveMessage(int master_count,int slave_count,int slaveItem_count,const QString &message ){
    ...
    if(slaveItemFolder)
    {
        ...
        if(slaveItem_count >= MARK_SLAVE_ITEM_INPUT-MARK_SLAVE_ITEM){
            //qDebug() << "slaveItem_count:" << QString::number(slaveItem_count);
            if(slaveItem_count <= MARK_SLAVE_ITEM_INPUT-MARK_SLAVE_ITEM){
