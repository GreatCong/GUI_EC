...
int MainFormView::Generate_UserCode(QString &path){
...
//        mDeviceTree->Add_LeftTree_Slaves(slave.m_slave_index,slave.m_name);
        ...
        foreach (Ethercat_SlaveMSG_Item input, slave.input_list) {
            ...
            txtOutput << "\t input_"+QString::number(num) + " = " + "0x"+ QString::number(input.m_addr_offset,16);
        ...
        foreach (Ethercat_SlaveMSG_Item output, slave.output_list) {
           // qDebug() << output.dump_data(true);
            txtOutput << "\t output_"+QString::number(num) + " = " + "0x"+QString::number(output.m_addr_offset,16);
