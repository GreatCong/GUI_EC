    ...
    if(Master_init(ifname)){
        if(Master_config(false) >0){
            ...
            while(EcatError){
                ...
                qDebug("[Master]:%s", ec_elist2string());
            }
            ...
            {
                qDebug("Not all slaves reached safe operational state.State=%d",ec_slave[0].state);
//               printf("Not all slaves reached safe operational state.\n");
               ...
               for(i = 1; i<=ec_slavecount ; i++)
               {
                  ...
                  {
                      qDebug("[Master]:Slave %d State=%2x StatusCode=%4x : %s\n",
                             i, ec_slave[i].state, ec_slave[i].ALstatuscode, ec_ALstatuscode2string(ec_slave[i].ALstatuscode));
//                     printf("Slave %d State=%2x StatusCode=%4x : %s\n",
//                        i, ec_slave[i].state, ec_slave[i].ALstatuscode, ec_ALstatuscode2string(ec_slave[i].ALstatuscode));
                      qDebug("[Master]:slave %d State=%d",i,ec_slave[i].state);
                      if (ec_slave[i].state == (EC_STATE_INIT + EC_STATE_ERROR))//在从站扫描的时候，切换到init状态会出现错误
                      {
                         printf("ERROR : slave %d is in INIT + ERROR, attempting ack.\n", i);
                         //ec_slave[i].state = (EC_STATE_INIT + EC_STATE_ACK);
                         //ec_writestate(i);
                         if (ec_reconfig_slave(i, EC_TIMEOUTSTATE)){//如果状态切换到safe_op出错,就重新配置下
...
//            Master_ChangeState(0,STATE_INIT);
            for(i = 1; i<=ec_slavecount ; i++){
                Master_ChangeState(i,STATE_SAFE_OP);
            }
...
/******* 以下函数与读取EEPROM相关的  ******/
///
/// \brief My_EthercatMaster::Master_siMap_sdo
/// \param slave
/// \return
///
...
///
/// \brief My_EthercatMaster::Master_si_PDOassign
/// \param slave
/// \param PDOassign
/// \param mapoffset
/// \param bitoffset
...
///
/// \brief My_EthercatMaster::dtype2string
/// \param dtype
/// \return
///
...
///
/// \brief My_EthercatMaster::SDO2string
/// \param slave
/// \param index
/// \param subidx
/// \param dtype
...
/** Read PDO assign structure */
///
/// \brief My_EthercatMaster::si_PDOassign
/// \param slave
/// \param PDOassign
/// \param mapoffset
/// \param bitoffset
...
///
/// \brief My_EthercatMaster::si_map_sdo
/// \param slave
/// \return
///
...
///
/// \brief My_EthercatMaster::si_siiPDO
/// \param slave
/// \param t
/// \param mapoffset
/// \param bitoffset
...
///
/// \brief My_EthercatMaster::si_map_sii
/// \param slave
/// \return
///
...
///
/// \brief My_EthercatMaster::si_sdo
/// \param cnt
///
